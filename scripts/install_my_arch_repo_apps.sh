...
if [[ "$choice" == "y" || "$choice" == "Y" ]]; then
    ...
    for pkg in steam lxsession lxappearance networkmanager network-manager-applet tailscale bluez bluez-utils \
               blueman pavucontrol pcmanfm gvfs gvfs-smb gvfs-mtp gvfs-afc xdg-desktop-portal \
    ...
    for pkg in ffmpeg avahi mpv peek cheese exiv2 audacity krita shotcut filezilla gthumb handbrake; do
        install_package "$pkg"
    done
